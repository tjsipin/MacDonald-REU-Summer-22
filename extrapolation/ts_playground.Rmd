...
for (i in 1:(unique(monthly_short[,'Code']) %>% nrow)){
  for (j in 1:h){
    for (k in 15:16){
      test_rwf_df_8[(i-1)*h + j, k] = (auto.arima(monthly_short %>% 
                                                    filter(Code == (unique(monthly_short[,'Code'])[i,] %>% as.integer())) %>% 
                                                    select(c())))
    }
  }
...
save(test_rwf_df_7, file = 'projected_input')
write.csv(test_rwf_df_7, file = 'projected_input.csv')
```
