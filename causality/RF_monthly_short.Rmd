...
preds_bind <- function(data_fit, lower = 0.05, upper = 0.95) {
  predict(
    ...
    extract_recipe(rf_wf) %>% 
      bake(data_fit),
    type = 'quantiles',
    quantiles = c(lower, upper, 0.05)
  ) %>% 
    with(predictions) %>% 
    as_tibble() %>%
    set_names(paste('.pred', c('_lower', '_upper', ''))) %>% 
    mutate(across(contains('.pred'), ~10^.x)) %>% 
    bind_cols(data_fit) %>% 
    select(contains('.pred'), CL, Year, Month, Population,
           LST_Day, NDVI, EVI, Precip, AvgRad, SWOccurrence,
           pland_forest, area_mn_forest, enn_mn_forest, te_forest)
}
...
set.seed(123)
rf_preds_test %>% 
  mutate(pred_interval = cut_interval(CL, )) +
