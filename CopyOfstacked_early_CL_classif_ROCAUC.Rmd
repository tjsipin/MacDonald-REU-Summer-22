...
data_test_30 <-  (aad %>% 
                    select(c(colnames(data_test_30_temp)))) %>% 
  slice(Index_temp) %>% 
...
conf_mat_early_CL_classif_30 <- caret::confusionMatrix(data = data_test_30_2$.pred_class, 
...
data_test_60 <-  (aad %>% 
                    select(c(colnames(data_test_60_temp)))) %>% 
  slice(Index_temp) %>% 
  subset(!is.na(LST_Day)) %>% 
  subset(!is.na(SWOccurrence))
...
data_test_70 <-  (aad %>% 
                    select(c(colnames(data_test_70_temp)))) %>% 
  slice(Index_temp) %>% 
  subset(!is.na(LST_Day)) %>% 
  subset(!is.na(SWOccurrence))
