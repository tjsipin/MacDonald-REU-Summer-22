...
data <- data %>%
  ...
  filter(!is.na(Malaria)) %>%
  filter(Malaria > 0) %>%
  dplyr::select(c("Population", "Malaria", "LST_Day", "Precip", "AvgRad", "SWOccurrence", "NDVI", "EVI", "pland_forest", "te_forest", "enn_mn_forest"))
...
data$Malaria <- as.factor(ifelse(data$Malaria < median, 0, 1))
...
caret::confusionMatrix(data_test$Malaria, 
...
# 2 accuracy binary         0.820 .pred_class            
# 3 accuracy binary         0.816 .pred_class_xgb_res_1_2
# 4 accuracy binary         0.827 .pred_class_rf_res_1_5 
