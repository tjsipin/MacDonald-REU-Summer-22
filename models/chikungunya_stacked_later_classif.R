...
aad <- subset(aad, !is.na(aad$Chikungunya))
aad <-subset(aad, aad$Chikungunya > 0)
summary(aad$Chikungunya)
...
later_data_small <- later_data %>%
  select(c("Population", "Chikungunya", "LST_Day", "Precip", "AvgRad", 
...
library(dplyr)
summary(later_data_small$Chikungunya)
cat_df <- later_data_small
cat_df$Chikungunya <- cut(later_data_small$Chikungunya, 
                                     breaks = c(0, 0.7165631, 10^3), 
                                     labels = c("low", "high")) # median


skimr::skim(cat_df)
round(prop.table(table(cat_df$Chikungunya)), 2)
...
data_rec <-
  recipe(Chikungunya ~ Population + LST_Day + NDVI + 
...
# add metric rmse (same as Chikungunya)
...
caret::confusionMatrix(data_test30$Chikungunya, 
                       data_test30$.pred_class,
                       positive = 'high')

plot(data_test70$Chikungunya, data_test70$.pred_class)


## fit a logistic regression to the data...
glm.fit30=glm(data_test30$.pred_class ~ data_test30$Chikungunya, family=binomial)
lines(data_test30$Chikungunya, glm.fit$fitted.values)

glm.fit40=glm(data_test40$.pred_class ~ data_test40$Chikungunya, family=binomial)
lines(data_test30$Chikungunya, glm.fit$fitted.values)

glm.fit50=glm(data_test50$.pred_class ~ data_test50$Chikungunya, family=binomial)
lines(data_test30$Chikungunya, glm.fit$fitted.values)

glm.fit60=glm(data_test60$.pred_class ~ data_test60$Chikungunya, family=binomial)
lines(data_test30$Chikungunya, glm.fit$fitted.values)

glm.fit70=glm(data_test70$.pred_class ~ data_test70$Chikungunya, family=binomial)
lines(data_test30$Chikungunya, glm.fit$fitted.values)
...
##
#######################################
rf.model <- randomForest(factor(data_test30$.pred_class) ~ data_test30$Chikungunya)
...
# <chr>    <chr>          <dbl> <chr>                  
#   1 accuracy binary         1     Chikungunya            
# 2 accuracy binary         0.654 .pred_class            
# 3 accuracy binary         0.560 .pred_class_svm_res_1_3
# 4 accuracy binary         0.605 .pred_class_svm_res_1_2
# 5 accuracy binary         0.605 .pred_class_xgb_res_1_3
# 6 accuracy binary         0.638 .pred_class_xgb_res_1_5
# 7 accuracy binary         0.646 .pred_class_rf_res_1_1 
