...
later_data_small <- later_data %>%
  dplyr::select(c("Year", "Population", "Cutaneous.Leishmaniasis", "LST_Day", "NDVI", "EVI", "Precip", "AvgRad", "SWOccurrence", "pland_forest", "te_forest", "enn_mn_forest"))
...
later_data_small$pland_forest <- ifelse(is.na(later_data_small$pland_forest), 0, later_data_small$pland_forest)
later_data_small$te_forest <- ifelse(is.na(later_data_small$te_forest), 0, later_data_small$te_forest)
later_data_small$enn_mn_forest <- ifelse(is.na(later_data_small$enn_mn_forest), 0, later_data_small$enn_mn_forest)
later_data_small$Year <- as.numeric(later_data_small$Year)
```
...
model_spec_default <- randomForest(train_data$Cutaneous.Leishmaniasis ~ .,train_data[-11])

plot(model_spec_default)
varImpPlot(model_spec_default, cex = 1, pch = 16)
```
...
without_output <- dplyr::select(training, -Cutaneous.Leishmaniasis)
...
# Accessing the tuning results
  bestParameters <- dplyr::select_best(result_tuning, metric = "roc_auc", maximize = TRUE)
# Finalize recipe
  final_rec <- 
    rec %>%
