---
title: "Later Data Visceral XGBoost2"
...
later_data_t <- subset(later_data_t, later_data_t$Visceral.Leishmaniasis >0)
```
...
round(prop.table(table(later_data_t$Visceral.Leishmaniasis)), 2)
```
...
library(Matrix)
library(dplyr)
data_split <- initial_split(later_data_t, prop = 0.8, strata = Visceral.Leishmaniasis)
...
sparse_matrix <- sparse.model.matrix(
  Visceral.Leishmaniasis ~ . - 1, data = train
) # use for data = ?


# split training into predictors and labels
x_train <- as.matrix(train %>%
                       dplyr::select(-c("Visceral.Leishmaniasis")))
...
# Create tasks
library(mlr)
traintask <- makeRegrTask(data = train, target = "Visceral.Leishmaniasis")
testtask <- makeRegrTask(data = test, target = "Visceral.Leishmaniasis")
