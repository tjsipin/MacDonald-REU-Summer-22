...
aad <- subset(aad, !is.na(aad$Visceral.Leishmaniasis))
aad <-subset(aad, aad$Visceral.Leishmaniasis > 0)
summary(aad$Visceral.Leishmaniasis)
...
later_data_small <- later_data %>%
  select(c("Population", "Visceral.Leishmaniasis", "LST_Day", "Precip", "AvgRad", 
...
library(dplyr)
summary(later_data_small$Visceral.Leishmaniasis)
cat_df <- later_data_small
cat_df$Visceral.Leishmaniasis <- cut(later_data_small$Visceral.Leishmaniasis, 
                                    breaks = c(0, 0.12777376, 10^3), 
                                    labels = c("low", "high")) # median


skimr::skim(cat_df)
round(prop.table(table(cat_df$Visceral.Leishmaniasis)), 2)
...
data_rec <-
  recipe(Visceral.Leishmaniasis ~ Population + LST_Day + NDVI + 
...
# add metric rmse (same as Visceral.Leishmaniasis)
...
caret::confusionMatrix(data_test30$Visceral.Leishmaniasis, 
                       data_test30$.pred_class,
                       positive = 'high')

plot(data_test70$Visceral.Leishmaniasis, data_test70$.pred_class)


## fit a logistic regression to the data...
glm.fit30=glm(data_test30$.pred_class ~ data_test30$Visceral.Leishmaniasis, family=binomial)
lines(data_test30$Visceral.Leishmaniasis, glm.fit$fitted.values)

glm.fit40=glm(data_test40$.pred_class ~ data_test40$Visceral.Leishmaniasis, family=binomial)
lines(data_test30$Visceral.Leishmaniasis, glm.fit$fitted.values)

glm.fit50=glm(data_test50$.pred_class ~ data_test50$Visceral.Leishmaniasis, family=binomial)
lines(data_test30$Visceral.Leishmaniasis, glm.fit$fitted.values)

glm.fit60=glm(data_test60$.pred_class ~ data_test60$Visceral.Leishmaniasis, family=binomial)
lines(data_test30$Visceral.Leishmaniasis, glm.fit$fitted.values)

glm.fit70=glm(data_test70$.pred_class ~ data_test70$Visceral.Leishmaniasis, family=binomial)
lines(data_test30$Visceral.Leishmaniasis, glm.fit$fitted.values)
...
##
#######################################
rf.model <- randomForest(factor(data_test30$.pred_class) ~ data_test30$Visceral.Leishmaniasis)
...
# <chr>    <chr>          <dbl> <chr>                  
#   1 accuracy binary         1     Visceral.Leishmaniasis 
# 2 accuracy binary         0.773 .pred_class            
# 3 accuracy binary         0.765 .pred_class_xgb_res_1_1
# 4 accuracy binary         0.737 .pred_class_xgb_res_1_3
# 5 accuracy binary         0.775 .pred_class_rf_res_1_5 
