...
aad <- subset(aad, !is.na(aad$Dengue))
aad <-subset(aad, aad$Dengue > 0)
summary(aad$Dengue)
...
later_data_small <- later_data %>%
  select(c("Population", "Dengue", "LST_Day", "Precip", "AvgRad", 
...
skimr::skim(cat_df)
round(prop.table(table(cat_df$Dengue)), 2)
...
data_rec <-
  recipe(Dengue ~ Population + LST_Day + NDVI + 
...
# add metric rmse (same as Dengue)
...
# <chr>    <chr>          <dbl> <chr>                  
#   1 accuracy binary         1     Dengue                 
# 2 accuracy binary         0.676 .pred_class            
# 3 accuracy binary         0.661 .pred_class_xgb_res_1_1
# 4 accuracy binary         0.665 .pred_class_xgb_res_1_2
# 5 accuracy binary         0.668 .pred_class_xgb_res_1_5
# 6 accuracy binary         0.659 .pred_class_rf_res_1_4 
# 7 accuracy binary         0.680 .pred_class_rf_res_1_5 
