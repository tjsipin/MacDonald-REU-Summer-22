...
data_rec <-
  recipe(Cutaneous.Leishmaniasis ~ LST_Day + NDVI + EVI + Precip + AvgRad + SWOccurrence + pland_forest + te_forest + enn_mn_forest + Population, data = data_train) %>%
...
save(svm_res, file = 'base_models/svm_res_later_CL_classif_40_imp')
...
save(xgb_res, file = 'base_models/xgb_res_later_CL_classif_40_imp')
# 
...
save(rf_res, file = 'base_models/rf_res_later_CL_classif_40_imp')
# 
...
save(nb_res, file = 'base_models/nb_res_later_CL_classif_40_imp')

load(file = 'base_models/svm_res_later_CL_classif_40_imp')
load(file = 'base_models/xgb_res_later_CL_classif_40_imp')
load(file = 'base_models/rf_res_later_CL_classif_40_imp')
load(file = 'base_models/nb_res_later_CL_classif_40_imp')
...
save(data_st_40, file = 'data_stacks/data_st_later_CL_classif_40_imp')
load('data_stacks/data_st_later_CL_classif_40_imp')
...
save(model_st_40, file = 'model_stacks/model_st_later_CL_classif_40_imp')

load('model_stacks/model_st_later_CL_classif_40_imp')

data_test_40 <-
  data_test_40 %>%
  bind_cols(predict(model_st_40, .))

save(data_test_40, file = 'data_tests/data_test_later_CL_classif_40_imp')

load(file = 'data_tests/data_test_later_CL_classif_40_imp')
# confusion matrix for stacks
...
save(conf_mat_later_CL_classif_40, file = 'confusion_matrices/conf_mat_later_CL_classif_40_imp')
...
data_test_50 <-  (aad %>% 
                    dplyr::select(c(colnames(data_test_50_temp))))[Index_temp,] %>% 
  subset(!is.na(LST_Day)) %>% 
  subset(!is.na(SWOccurrence))
...
data_test_60 <-  (aad %>% 
                    dplyr::select(c(colnames(data_test_60_temp))))[Index_temp,] %>% 
  subset(!is.na(LST_Day)) %>% 
  subset(!is.na(SWOccurrence))
...
data_test_70 <-  (aad %>% 
                    dplyr::select(c(colnames(data_test_70_temp))))[Index_temp,]%>%
  subset(!is.na(LST_Day)) %>% 
  subset(!is.na(SWOccurrence))
