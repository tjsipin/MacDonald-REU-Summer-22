...
mean_Cutaneous <- monthly_df %>%
  ...
  summarise_at(vars(Cutaneous.Leishmaniasis), list(name = mean))
mean_Cutaneous
plot(mean_Cutaneous, main = "Cutaneous Cases between 2001 and 2019", xlab = "Month", ylab = "Cases")

fit <- lm(monthly_df$Cutaneous.Leishmaniasis ~ monthly_df$Month, data = monthly_df)
summary(fit)
abline(fit, col = "red")
```
