...
median_data <- data %>%
  filter(!is.na(Chikungunya)) %>%
  filter(Chikungunya > 0) %>%
  dplyr :: select(Chikungunya)

median <- median_data$Chikungunya %>%
  median()
...
data <- data %>%
  ...
  filter(!is.na(Chikungunya)) %>%
  filter(Chikungunya > 0) %>%
  dplyr::select(c("Population", "Chikungunya", "LST_Day", "Precip", "AvgRad", "SWOccurrence", "NDVI", "EVI", "pland_forest", "te_forest", "enn_mn_forest"))
...
data$Chikungunya <- as.factor(ifelse(data$Chikungunya < median, 0, 1))
...
caret::confusionMatrix(data_test$Chikungunya, 
...
# 2 accuracy binary         0.665 .pred_class            
# 3 accuracy binary         0.609 .pred_class_svm_res_1_3
# 4 accuracy binary         0.677 .pred_class_xgb_res_1_1
# 5 accuracy binary         0.664 .pred_class_xgb_res_1_3
# 6 accuracy binary         0.665 .pred_class_rf_res_1_4 
# 7 accuracy binary         0.665 .pred_class_rf_res_1_1 
