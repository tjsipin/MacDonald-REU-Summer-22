---
title: "Later_Data_Malaria_XGBoost2"
...
later_data_t <- later_data_small %>%
  mutate(Malaria = (Malaria))
```

```{r}
library(dplyr)
summary(later_data_small$Malaria[later_data_small$Malaria > 0])
cat_df <- subset(later_data_small, later_data_small$Malaria > 0)
cat_df$Malaria <- cut(later_data_small$Malaria[later_data_small$Malaria > 0], breaks = c(0, 0.0522, 1.3282, 10^3), labels = c("low", "moderate", "high")) # 25% 75% 100%
...
```{r}
set.seed(1)
data_split <- initial_split(cat_df, strata = "Malaria", prop = 0.75)
...
y_train <- as.vector(as.numeric(training$Malaria) - 1)
y_test <- as.vector(as.numeric(testing$Malaria) - 1)
...
training$Malaria <- as.factor(training$Malaria)
testing$Malaria <- as.factor(testing$Malaria)

#install.packages("mlr")
library(mlr)

# Create tasks
traintask <- makeClassifTask(data = training, target = "Malaria")
testtask <- makeClassifTask(data = testing, target = "Malaria")
