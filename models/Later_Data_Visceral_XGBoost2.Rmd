---
title: "Later Data Visceral XGBoost"
...
new_data <- subset(aad, !is.na(visceral))
#View(new_data)
#names(new_data)
```
...
later_data_small <- later_data %>%
  dplyr::select(c("Population", "Visceral.Leishmaniasis", "Dengue_Alb_OptTemp", "Dengue_Aeg_OptTemp", "Chik_Alb_OptTemp", "Chik_Aeg_OptTemp", "Zika_OptTemp", "Malaria_OptTemp", "LST_Day", "LST_Night", "OptTemp_Obs", "NDVI", "EVI", "Precip", "AvgRad", "SWOccurrence")) %>%
  na.omit(later_data_small) 
later_data_t <- later_data_small %>%
  mutate(Visceral.Leishmaniasis = (Visceral.Leishmaniasis))
```

```{r}
library(dplyr)
summary(later_data_small$Visceral.Leishmaniasis[later_data_small$Visceral.Leishmaniasis > 0])
cat_df <- subset(later_data_small, later_data_small$Visceral.Leishmaniasis > 0)
cat_df$Visceral.Leishmaniasis <- cut(later_data_small$Visceral.Leishmaniasis[later_data_small$Visceral.Leishmaniasis > 0], breaks = c(0, 0.049173 , 0.195734, 10^3), labels = c("low", "moderate", "high")) # 25% 75% 100%
...
set.seed(1)
library(xgboost)
data_split <- initial_split(cat_df, strata = "Visceral.Leishmaniasis", prop = 0.8)

training <- training(data_split) %>%
  group_by(Visceral.Leishmaniasis) %>%
  sample_n(size = 928)
...
y_train <- as.vector(as.numeric(training$Visceral.Leishmaniasis) - 1)
y_test <- as.vector(as.numeric(testing$Visceral.Leishmaniasis) - 1)
...
training$Visceral.Leishmaniasis <- as.factor(training$Visceral.Leishmaniasis)
testing$Visceral.Leishmaniasis <- as.factor(testing$Visceral.Leishmaniasis)

#install.packages("mlr")
library(mlr)

# Create tasks
traintask <- makeClassifTask(data = training, target = "Visceral.Leishmaniasis")
testtask <- makeClassifTask(data = testing, target = "Visceral.Leishmaniasis")
