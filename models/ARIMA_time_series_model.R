...
tidy_df <- aad %>%
  dplyr::select(c("Year", "Population", "Cutaneous.Leishmaniasis", "LST_Day", "Precip", "SWOccurrence", "NDVI", "EVI", "pland_forest", "te_forest", "enn_mn_forest"))
...
Cutaneous.Leishmaniasis <- ts(tidy_df$Cutaneous.Leishmaniasis, 
                              start = c(2000), frequency = 12)
dCutaneous.Leishmaniasis <- decompose(Cutaneous.Leishmaniasis, 
                                      "multiplicative")
plot(dCutaneous.Leishmaniasis)
Population <- ts(tidy_df$Population, start = c(2000), 
                 frequency = 1)
