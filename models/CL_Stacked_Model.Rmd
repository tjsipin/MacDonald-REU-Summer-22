...
Separating the data into two separate data sets, one for data before 2014 (early_data) and one for data after 2013 (later_data), and then tidying it while removing unnecessary columns. 
```{r}
library(dplyr)
early_data <- aad %>%
  filter(Year < 2014)
...
Converting all NA values in pland_forest, te_forest, and enn_mn_forest to 0.
```{r}
...
data_train <-subset(data_train, data_train$Population < 4000000)
data_train <-subset(data_train, data_train$Precip < 6000)
data_train <-subset(data_train, data_train$LST_Day < 37)
data_train <-subset(data_train, data_train$LST_Day > 17)
data_train <-subset(data_train, data_train$AvgRad < 50)
data_train <-subset(data_train, data_train$NDVI > 0.2)
data_train <-subset(data_train, data_train$EVI > 0.1)
data_train <-subset(data_train, data_train$pland_forest > 0)
data_train <-subset(data_train, data_train$te_forest > 0)
data_train <-subset(data_train, data_train$enn_mn_forest > 0)
data_train <-subset(data_train, data_train$enn_mn_forest < 3000)
...
caret::confusionMatrix(data_test$Cutaneous.Leishmaniasis,data_test$.pred_class, positive = 'high')
```
