...
median_data <- data %>%
  filter(!is.na(Mucosal.Leishmaniasis)) %>%
  filter(Mucosal.Leishmaniasis > 0) %>%
  dplyr :: select(Mucosal.Leishmaniasis)

median <- median_data$Mucosal.Leishmaniasis %>%
  median()
...
data <- data %>%
  ...
  filter(!is.na(Mucosal.Leishmaniasis)) %>%
  filter(Mucosal.Leishmaniasis > 0) %>%
  dplyr::select(c("Population", "Mucosal.Leishmaniasis", "LST_Day", "Precip", "AvgRad", "SWOccurrence", "NDVI", "EVI", "pland_forest", "te_forest", "enn_mn_forest"))
...
data$Mucosal.Leishmaniasis <- as.factor(ifelse(data$Mucosal.Leishmaniasis < median, 0, 1))
...
caret::confusionMatrix(data_test$Mucosal.Leishmaniasis, 
...
# 2 accuracy binary         0.838 .pred_class            
# 3 accuracy binary         0.838 .pred_class_xgb_res_1_2
# 4 accuracy binary         0.834 .pred_class_rf_res_1_5 
# 5 accuracy binary         0.834 .pred_class_rf_res_1_3
