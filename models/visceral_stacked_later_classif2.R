...
median <- median_data$Visceral.Leishmaniasis %>%
  median()
...
data <- data %>%
  ...
  filter(!is.na(Visceral.Leishmaniasis)) %>%
  filter(Visceral.Leishmaniasis > 0) %>%
  dplyr::select(c("Population", "Visceral.Leishmaniasis", "LST_Day", "Precip", "AvgRad", "SWOccurrence", "NDVI", "EVI", "pland_forest", "te_forest", "enn_mn_forest"))
...
data$Visceral.Leishmaniasis <- as.factor(ifelse(data$Visceral.Leishmaniasis < median, 0, 1))
...
# 2 accuracy binary         0.783 .pred_class            
# 3 accuracy binary         0.729 .pred_class_xgb_res_1_3
# 4 accuracy binary         0.769 .pred_class_rf_res_1_4 
# 5 accuracy binary         0.786 .pred_class_rf_res_1_5 
