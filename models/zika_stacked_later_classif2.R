...
data <- data %>%
  ...
  filter(!is.na(Zika)) %>%
  filter(Zika > 0) %>%
  dplyr::select(c("Population", "Zika", "LST_Day", "Precip", "AvgRad", "SWOccurrence", "NDVI", "EVI", "pland_forest", "te_forest", "enn_mn_forest"))
...
data$Zika <- as.factor(ifelse(data$Zika < median, 0, 1))
...
# 2 accuracy binary         0.656 .pred_class            
# 3 accuracy binary         0.665 .pred_class_xgb_res_1_3
# 4 accuracy binary         0.65  .pred_class_rf_res_1_1 
# 5 accuracy binary         0.653 .pred_class_rf_res_1_2 
