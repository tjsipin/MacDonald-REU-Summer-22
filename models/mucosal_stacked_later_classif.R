...
aad <- subset(aad, !is.na(aad$Mucosal.Leishmaniasis))
aad <-subset(aad, aad$Mucosal.Leishmaniasis > 0)
summary(aad$Mucosal.Leishmaniasis)
...
later_data_small <- later_data %>%
  select(c("Population", "Mucosal.Leishmaniasis", "LST_Day", "Precip", "AvgRad", 
...
library(dplyr)
summary(later_data_small$Mucosal.Leishmaniasis)
cat_df <- later_data_small
cat_df$Mucosal.Leishmaniasis <- cut(later_data_small$Mucosal.Leishmaniasis, 
                      breaks = c(0, 0.2588272, 10^3), 
...
data_rec <-
  recipe(Mucosal.Leishmaniasis ~ Population + LST_Day + NDVI + 
...
# add metric rmse (same as Mucosal.Leishmaniasis)
...
caret::confusionMatrix(data_test30$Mucosal.Leishmaniasis, 
                       data_test30$.pred_class,
                       positive = 'high')

plot(data_test30$Mucosal.Leishmaniasis, data_test30$.pred_class)


## fit a logistic regression to the data...
glm.fit30=glm(data_test30$.pred_class ~ data_test30$Mucosal.Leishmaniasis, family=binomial)
lines(data_test30$Mucosal.Leishmaniasis, glm.fit$fitted.values)

glm.fit40=glm(data_test40$.pred_class ~ data_test40$Mucosal.Leishmaniasis, family=binomial)
lines(data_test30$Mucosal.Leishmaniasis, glm.fit$fitted.values)

glm.fit50=glm(data_test50$.pred_class ~ data_test50$Mucosal.Leishmaniasis, family=binomial)
lines(data_test30$Mucosal.Leishmaniasis, glm.fit$fitted.values)

glm.fit60=glm(data_test60$.pred_class ~ data_test60$Mucosal.Leishmaniasis, family=binomial)
lines(data_test30$Mucosal.Leishmaniasis, glm.fit$fitted.values)

glm.fit70=glm(data_test70$.pred_class ~ data_test70$Mucosal.Leishmaniasis, family=binomial)
lines(data_test30$Mucosal.Leishmaniasis, glm.fit$fitted.values)
...
##
#######################################
rf.model <- randomForest(factor(data_test30$.pred_class) ~ data_test30$Mucosal.Leishmaniasis)
...
# <chr>    <chr>          <dbl> <chr>                  
#   1 accuracy binary         1     Mucosal.Leishmaniasis  
# 2 accuracy binary         0.865 .pred_class            
# 3 accuracy binary         0.846 .pred_class_xgb_res_1_5
# 4 accuracy binary         0.846 .pred_class_rf_res_1_1 
# 5 accuracy binary         0.808 .pred_class_rf_res_1_3
