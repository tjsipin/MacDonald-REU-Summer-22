...
later_data_small <- later_data %>%
  select(c("Cutaneous.Leishmaniasis", "Dengue_Alb_OptTemp", "Dengue_Aeg_OptTemp", "Chik_Alb_OptTemp", "Chik_Aeg_OptTemp", "Zika_OptTemp", "Malaria_OptTemp", "LST_Day", "LST_Night", "OptTemp_Obs", "NDVI", "EVI", "Precip", "AvgRad", "SWOccurrence")) %>%
  na.omit(later_data_small) 
later_data_t <- lapply(later_data_small, as.numeric)
later_data_t <- as.data.frame(later_data_t)
later_data_t <- subset(later_data_t, later_data_t$Cutaneous.Leishmaniasis >0 & later_data_t$Cutaneous.Leishmaniasis < 45)
```
...
round(prop.table(table(later_data_t$Cutaneous.Leishmaniasis)), 2)
```

```{r}
# splitting data into training and test data
set.seed(21)
data_split <- initial_split(later_data_t, prop = 0.9, strata = Cutaneous.Leishmaniasis)
...
x_train[,1:14] = as.numeric(x_train[,1:14])
...
x_test[,1:14] = as.numeric(x_test[,1:14])
