...
aad <- subset(aad, !is.na(aad$Zika))
aad <-subset(aad, aad$Zika > 0)
summary(aad$Zika)
...
later_data_small <- later_data %>%
  select(c("Population", "Zika", "LST_Day", "Precip", "AvgRad", 
...
library(dplyr)
summary(later_data_small$Zika)
cat_df <- later_data_small
cat_df$Zika <- cut(later_data_small$Zika, 
                     breaks = c(0, 0.5905902, 10^3), 
                     labels = c("low", "high")) # median


skimr::skim(cat_df)
round(prop.table(table(cat_df$Zika)), 2)
...
data_rec <-
  recipe(Zika ~ Population + LST_Day + NDVI + 
...
caret::confusionMatrix(data_test70$Zika, 
                       data_test70$.pred_class,
                       positive = 'high')


plot(data_test70$Zika, data_test70$.pred_class)


## fit a logistic regression to the data...
glm.fit30=glm(data_test30$.pred_class ~ data_test30$Zika, family=binomial)
lines(data_test30$Zika, glm.fit$fitted.values)

glm.fit40=glm(data_test40$.pred_class ~ data_test40$Zika, family=binomial)
lines(data_test30$Zika, glm.fit$fitted.values)

glm.fit50=glm(data_test50$.pred_class ~ data_test50$Zika, family=binomial)
lines(data_test30$Zika, glm.fit$fitted.values)

glm.fit60=glm(data_test60$.pred_class ~ data_test60$Zika, family=binomial)
lines(data_test30$Zika, glm.fit$fitted.values)

glm.fit70=glm(data_test70$.pred_class ~ data_test70$Zika, family=binomial)
lines(data_test30$Zika, glm.fit$fitted.values)
...
##
#######################################
rf.model <- randomForest(factor(data_test30$.pred_class) ~ data_test30$Zika)
...
# <chr>    <chr>          <dbl> <chr>                  
#   1 accuracy binary         1     Zika                   
# 2 accuracy binary         0.669 .pred_class            
# 3 accuracy binary         0.564 .pred_class_svm_res_1_2
# 4 accuracy binary         0.624 .pred_class_xgb_res_1_2
# 5 accuracy binary         0.666 .pred_class_xgb_res_1_3
# 6 accuracy binary         0.666 .pred_class_rf_res_1_1 
