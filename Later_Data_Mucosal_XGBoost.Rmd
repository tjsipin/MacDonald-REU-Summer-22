---
title: "Later Data Mucosal XGBoost"
...
later_data_t <- subset(later_data_t, later_data_t$Mucosal.Leishmaniasis >0 & later_data_t$Mucosal.Leishmaniasis <3.5)
```
...
round(prop.table(table(later_data_t$Mucosal.Leishmaniasis)), 2)
```

```{r}
# splitting data into training and test data
set.seed(21)
data_split <- initial_split(later_data_t, prop = 0.8, strata = Mucosal.Leishmaniasis)
...
sparse_matrix <- sparse.model.matrix(
  Mucosal.Leishmaniasis ~ . - 1, data = train
) # use for data = ?


# split training into predictors and labels
x_train <- as.matrix(train %>%
                       dplyr::select(-c("Mucosal.Leishmaniasis")))
...
```{r}
# Create tasks
traintask <- makeRegrTask(data = train, target = "Mucosal.Leishmaniasis")
testtask <- makeRegrTask(data = test, target = "Mucosal.Leishmaniasis")
